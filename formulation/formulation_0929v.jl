...
function formulation(epsilon::Int64, cutting_num::Int64, N::Vector{Int64}, F::Vector{Int64}, G::Vector{Int64}, R::Vector{Int64}, H::Vector{Int64}, K::Vector{Int64}, Return_TMC::Vector{Int64}, service::Vector{Int64}, 
    dis::Matrix{Float64}, time::Matrix{Float64}, patient::Vector{Int64}, demand::Vector{Int64}, capacity_vg::Int64, capacity_vr::Int64, capacity_room::Vector{Int64}, capacity_man::Vector{Int64}, EC_G::Int64, EC_R::Int64, VC_G::Int64, VC_R::Int64, FC::Int64, M::Int64, WG::Int64, WR::Int64)
    ...
    for i in F
        for j in vcat(G, R)
            @constraint(model, time[i, j] <= b[j] + M * (1 - sum(x[i, j, k] for k in K)))
        end
    end
    ...
    for i in G
        for j in G
            if i != j
                @constraint(model, b[i] + service[i] + time[i, j] <= b[j] + M * (1 - sum(x[i, j, k] for k in K)))
            end
        end
    end
    ...
    # for i in R
    #     for h in H
    #         @constraint(model, demand[i] * u[i, h] + patient[i] <= capacity_vr)
    #     end
    # end

    # Constraint 25
    for h in H
        @constraint(model, sum(u[i, h]*patient[i] for i in R) <= capacity_room[h-cutting_num+length(H)])
    end

    # Constraint 26
    for h in H
        @constraint(model, sum(u[i, h]*demand[i] for i in R) <= capacity_man[h-cutting_num+length(H)])
    end
...
function run_math(problem::String, coordinates::Vector{Vector}, cutting_num::Int64, TMC_num::Int64, green_num::Int64, red_num::Int64, hospital_num::Int64, 
    ...
    capacity_vg::Int64, capacity_vr::Int64, capacity_room::Vector{Int64}, capacity_man::Vector{Int64}, EC_G::Int64, EC_R::Int64, VC_G::Int64, VC_R::Int64, FC::Int64, M::Int64,
    epsilon::Int64, instance_str::String, formulation_version::String)
    
    model = formulation(epsilon, cutting_num, N, F, G, R, H, K, return_tmc, service, distance_matr, time_matr, patient, demand, capacity_vg, capacity_vr, capacity_room, capacity_man, EC_G, EC_R, VC_G, VC_R, FC, M, WG, WR)
