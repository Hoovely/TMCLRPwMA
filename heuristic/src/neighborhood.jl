...
function N1!(chrm::Chromosome, TT::Matrix{Float64}, close_nodes::Matrix{Bool}, service::Vector{Int64}, patient::Vector{Int64}, demand::Vector{Int64}, capacity_v::Int64)   #Shift(0,1)
    ...
    elseif nt == 2
        ...
        if close_nodes[depot1, city1] || close_nodes[city1, tour2[1]]
            push!(candidates, 1)
        end
        for i = 2:nt
        ...
        if close_nodes[depot1, city1] || close_nodes[city1, tour2[nt]] 
            push!(candidates, nt + 1)
        end
    end
    ...
    new_time2 = calculate_new_cost_add_one(tour2, depot2, time2, city1, k2, TT) + service[city1]
    new_time1 = calculate_new_cost_remove_one(tour1, depot1, time1, k1, TT) - service[city1]
    ...
    if capacity_v - max(tours[r2].staff, demand[city1]) < sum(tours[r2].patient) + patient[city1] 
        return 
    end
...
function Improve_chromosome!(chrm::Chromosome, TT::Matrix{Float64}, close_nodes::Matrix{Bool}, n_nodes::Int, roullet::Vector{Int}, old_best::Float64, iterations::Int,
    ...
    # println("Start Improve_chromosome!")
    for i in 1:iterations
        r = sample(1:length(roullet), weights(roullet)) # at first iteration, randomly selecting 1~4 
        ...
        elseif r == 2
            Ni2!(chrm, TT, close_nodes, service)
        elseif r == 3
        ...
        elseif r == 6
            N1!(chrm, TT, close_nodes, service, patient, demand, capacity_v)
        else
