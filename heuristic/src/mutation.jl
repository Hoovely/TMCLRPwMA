...
function put_city_in_short_tour(c::Vector{Tour}, city::Int, T::Matrix{Float64}, service::Vector{Int64}, patient::Vector{Int64}, demand::Vector{Int64}, capacity_v::Int64)
    ...
    penalties = (time_lst)/maximum(time_lst) .+1

    for i = 1:length(c)
        tour = c[i].sequence[2:end]
        depot = c[i].sequence[1]
        nt = length(tour)
        ...
        if nt == 0 # destroied tour
            ...
            real_increase = T[depot, city] + service[city] + T[city, tour[1]] - T[depot, tour[1]]
            ...
            for j = 2:nt
                real_increase = T[tour[j-1], city] + service[city] + T[city, tour[j]] - T[tour[j-1], tour[j]]
                increase = penalties[i] * real_increase
                if increase < least_increase
                    least_increase = increase
                    best_tour = i
...
function rearange_nodes(chrm_::Chromosome, T::Matrix{Float64}, distance::Matrix{Float64}, service::Vector{Int64}, patient::Vector{Int64}, demand::Vector{Int64}, 
    capacity_v::Int64, FC::Int64, EC_G::Int64, VC_G::Int64, WG::Int64, WR::Int64, epsilon::Int64)
    ...
    for tour in c
        ...
        if !isempty(dlt_idx)
            ...
            if length(dlt_idx) == length(tour.sequence[2:end])
                tour.time = 0.0
                tour.sequence = Int[tour.sequence[1]]
            else
                # remove_cities_from_one_tour(tour, dlt_idx.-1, T)
    ...
    for node in deleted_nodes
        if put_city_in_short_tour(c, node, T, service, patient, demand, capacity_v) == false
            flag = false
            break
        end
    end
    ...
    if flag
        ...
        for (index, tour) in enumerate(c)
            ...
            push!(chrm.genes, tour.sequence[1])
            push!(chrm.genes, -1)
        ...
        deleteat!(c, delete_tours)
        chrm.weighted_time = WG*obj+WR*chrm.r_obj
        chrm.fitness = obj
        chrm.total_cost = calculate_costs(c, chrm.reds_path, distance, FC, EC_G, VC_G) + chrm.r_cost

        if chrm.total_cost > epsilon
            return chrm_
        end
...
function destroy_and_build(chrm_::Chromosome, T::Matrix{Float64}, distance::Matrix{Float64}, service::Vector{Int64}, patient::Vector{Int64}, demand::Vector{Int64}, 
    capacity_v::Int64, FC::Int64, EC_G::Int64, VC_G::Int64, WG::Int64, WR::Int64)
    # println("Start destroy_and_build")
    c = deepcopy(chrm_.tours)

    r1 = argmax([tour.time for tour in c])

    deleted_nodes = copy(c[r1].sequence)
    deleted_depot, deleted_patients = deleted_nodes[1], deleted_nodes[2:end]
    c[r1].sequence = Int[deleted_depot]
    c[r1].time = 0.0
    
    if isempty(deleted_patients)
        return chrm_
    end

    flag = true
    for node in deleted_patients
        if put_city_in_short_tour(c, node, T, service, patient, demand, capacity_v) == false
            flag = false
            break
        end
    ...
    if flag
        ...
        if chrm.total_cost > 250000
            println("Destory Build")
...
function mutate(chrm::Chromosome, T::Matrix{Float64}, distance::Matrix{Float64}, service::Vector{Int64}, patient::Vector{Int64}, demand::Vector{Int64}, 
    capacity_v::Int64, FC::Int64, EC_G::Int64, VC_G::Int64, WG::Int64, WR::Int64, epsilon::Int64)
    ...
    # println(new_chrm)
    if rand() < 0.0
        return destroy_and_build(new_chrm, T, distance, service, patient, demand, capacity_v, FC, EC_G, VC_G, WG, WR)
    else
        return rearange_nodes(new_chrm, T, distance, service, patient, demand, capacity_v, FC, EC_G, VC_G, WG, WR, epsilon)
    end
end
...
function find_tour_staff(tt::Vector{Int}, demand::Vector{Int64})
    return maximum([demand[node] for node in tt])
end
    
function find_tour_patient(tt::Vector{Int}, patient::Vector{Int64})
    return [patient[node] for node in tt]
end 

function find_tour_operation_cost(tt::Vector{Int}, distance::Matrix{Float64}, staff::Int64, EC_G::Int64, VC_G::Int64)
    
    operation_cost = VC_G + staff * EC_G
    length_trip = length(tt)
    for (index, node) in enumerate(tt)
        if index == length_trip
            next_node = tt[1]
            operation_cost += distance[node, next_node]
        else
            next_node = tt[index+1]
            operation_cost += distance[node, next_node]
        end
    end
