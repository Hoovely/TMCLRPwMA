...
function calculate_facility_score(f::Int64, WG::Int64, WR::Int64, distance::Matrix{Float64}, G::Vector{Int64}, R::Vector{Int64})
    
    score_g = 0
    for g in G
        score_g += distance[f, g]
    end

    score_r = 0
    for r in R
        score_r += distance[f, r]
    end

    return score_g*WG + score_r*WR
end

function greedy_solve_location_problem(p::Int64, WG::Int64, WR::Int64, distance::Matrix{Float64}, F::Vector{Int64}, G::Vector{Int64}, R::Vector{Int64}, FC::Int64)
    
    score = Vector{Float64}()
    score_dict = Dict{Float64, Int64}()
    for f in F
        value = calculate_facility_score(f, WG, WR, distance, G, R)
        push!(score, value)
        score_dict[value] = f
    end
    sort!(score, rev=true)
    ...
    F_ = Vector{Int64}()
    for value in score[begin:p]
        push!(F_, score_dict[value])
    end 
...
function greedy_solve_assignment_problem(distance::Matrix{Float64}, F_::Vector{Int64}, G::Vector{Int64}, R::Vector{Int64})
...
function greedy_solve_routing_problem(node_size::Int64, F_::Vector{Int64}, H::Vector{Int64}, assigned_cluster_g::Dict{Int64, Vector}, assigned_cluster_r::Dict{Int64, Vector}, WG::Int64, WR::Int64,
